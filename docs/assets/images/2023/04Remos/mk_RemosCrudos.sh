...
if [[ -n ${SRC_DIR} && -e ${SRC_DIR} && -d ${SRC_DIR} ]]; then
  ...
  if [ -d ${DEST_DIR} ]; then
    for SRC_IMAGE in \
        "${SRC_DIR}/20230415_093655.jpeg" \
        "${SRC_DIR}/IMG_0803.jpeg"; do
      CURF=${SRC_IMAGE##*/}
    ...
    magick montage -tile 2x \
      -shadow -geometry +6+6 \
      IMG_0803.jpeg \
      20230415_093655.jpeg \
      RemosCrudos_full.jpeg
    convert RemosCrudos_full.jpeg -resize ${size} RemosCrudos.jpeg
    rm IMG_0803.jpeg
    rm 20230415_093655.jpeg
    rm RemosCrudos_full.jpeg
    cd ${curd}
  else
